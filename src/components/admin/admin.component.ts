...
import { Question, QuizOption } from '../../models/quiz.model';
...
export class AdminComponent {
  ...
  // Partial<Question> permette dati incompleti durante la modifica
  // `index` traccia se si tratta di una modifica o di una nuova domanda
  editingQuestion = signal<Partial<Question> & { index?: number } | null>(null);
  ...
  startEdit(question: Question, index: number): void {
  ...
  updateQuestionText(event: Event): void {
    const target = event.target as HTMLInputElement;
    this.editingQuestion.update(q => q ? { ...q, questionText: target.value } : null);
  }

  updateQuestionImageUrl(event: Event): void {
    const target = event.target as HTMLInputElement;
  ...
  updateOptionText(index: number, event: Event): void {
    ...
    this.editingQuestion.update(q => {
      ...
      const newOptions = JSON.parse(JSON.stringify(q.options)) as QuizOption[];
      newOptions[index].text = target.value;
      return { ...q, options: newOptions };
    });
  }

  updateOptionImageUrl(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.editingQuestion.update(q => {
      if (!q || !q.options) return null;
      const newOptions = JSON.parse(JSON.stringify(q.options)) as QuizOption[];
