...
type QuizState = 'notStarted' | 'loading' | 'inProgress' | 'finished' | 'error' | 'admin' | 'categorySelection' | 'settings' | 'login';
...
export class QuizService {
  ...
  private firestoreIds = new Map<number, string>(); // Mappa l'indice dell'array all'ID del documento Firestore
  // --- FINE SEZIONE FIREBASE ---

  private readonly DEFAULT_QUIZ_DURATION_SECONDS = 45 * 60;

  quizState = signal<QuizState>('notStarted');
  ...
  private async loadQuestions(): Promise<void> {
    if (USE_FIREBASE) {
      ...
      //   const plainQuestions: Question[] = questionsFromDb.map((q, index) => {
  ...
  async generateQuizWithAI(total: number = 10): Promise<void> {
    // Stub
  }
  ...
  startQuiz(length: number, duration: number): void {
  ...
  async updateQuestion(updatedQuestion: Question, index: number): Promise<void> {
    if (USE_FIREBASE) {
      // --- SEZIONE FIREBASE (Disattivata) ---
